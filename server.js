...
//  AUTH
// ═══════════════════════════════════════════════════════════════════════════════
app.post('/api/register/editor', async (req, res) => {
...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    await sendEmail('socialmitr.company@gmail.com', {
      subject: `[SocialMitr Contact] ${subject} — from ${name}`,
      html: `
        <div style="font-family:'Segoe UI',sans-serif;background:#0f0f0f;color:#f0ece4;padding:32px;border-radius:12px;max-width:560px">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#1e1e1e;border:1px solid #2a2a2a;border-radius:10px;overflow:hidden">
            <tr><td style="padding:12px 18px;border-bottom:1px solid #2a2a2a"><span style="font-size:11px;color:#555;display:block">FROM</span><span style="font-weight:600">${name} (${email})</span></td></tr>
            <tr><td style="padding:12px 18px;border-bottom:1px solid #2a2a2a"><span style="font-size:11px;color:#555;display:block">TYPE</span><span style="font-weight:600">${type||'General'}</span></td></tr>
            <tr><td style="padding:12px 18px;border-bottom:1px solid #2a2a2a"><span style="font-size:11px;color:#555;display:block">SUBJECT</span><span style="font-weight:600">${subject}</span></td></tr>
            <tr><td style="padding:12px 18px"><span style="font-size:11px;color:#555;display:block;margin-bottom:8px">MESSAGE</span><p style="color:#aaa;line-height:1.7;margin:0">${message}</p></td></tr>
          </table>
    ...
    sendEmail(email, {
      subject: `We got your message, ${name}! — SocialMitr`,
      html: `<div style="font-family:'Segoe UI',sans-serif;background:#0f0f0f;color:#f0ece4;padding:32px;border-radius:12px;max-width:560px"><h2 style="color:#ff6b2b">Hey ${name}! 👋</h2><p style="color:#aaa;margin-top:12px;line-height:1.7">Thanks for reaching out to SocialMitr. We've received your message and will get back to you at <strong style="color:#ff6b2b">${email}</strong> within 24 hours.</p><p style="color:#555;font-size:12px;margin-top:20px">SocialMitr · Delhi, India · socialmitr.company@gmail.com</p></div>`,
      text: `Hi ${name}, thanks for contacting SocialMitr! We'll reply within 24 hours.`
    });
...
app.get('/api/admin/stats', async (req, res) => {
  ...
  if (secret !== (process.env.ADMIN_SECRET || 'socialmitr-admin-2026'))
...
app.post('/api/jobs/:id/apply', auth, async (req, res) => {
  ...
  try {
    ...
    sendEmail(bizEmail, applicationNotificationEmail({ businessName:job.businesses.business_name, ownerName:job.businesses.owner_name, jobTitle:job.title, creatorName:creator.name, creatorCity:creator.city, creatorSkill:creator.primary_skill, creatorRate:creator.rate, coverLetter:cover_letter||'No message.', dashboardLink:`${req.protocol}://${req.get('host')}/business-dashboard.html` }));
...
app.post('/api/contracts/:id/review', auth, async (req, res) => {
  ...
  try {
    const { data: contract } = await supabase.from('contracts').select('creator_id,business_id').eq('id',req.params.id).single();
    ...
    sendEmail(bizEmail, {
      ...
      html: `<div style="font-family:'Segoe UI',sans-serif;background:#0f0f0f;color:#f0ece4;padding:32px;border-radius:12px;max-width:560px">
        ...
        <div style="background:#1e1e1e;border:1px solid #2a2a2a;border-radius:10px;padding:20px;margin-bottom:20px">
          ...
          <p style="color:#ccc;font-size:14px;line-height:1.7;margin:0">${description}</p>
          ${link ? `<a href="${link}" style="display:inline-block;margin-top:12px;color:#ff6b2b;font-size:13px">🔗 View Deliverable</a>` : ''}
        </div>
      ...
      text: `${creator.name} delivered work for "${jobTitle}". ${link ? 'Link: '+link : ''}\n\n${description}`
    });
...
app.post('/api/ai/job-description', auth, async (req, res) => {
...
// ═══════════════════════════════════════════════════════════════════════════════
app.post('/api/ai/suggest-price', auth, async (req, res) => {
